...
-- -----------------------------------------------------
-- 0. GESTIÓN DE MULTI-TENANCY
-- -----------------------------------------------------
-- Tabla para las distintas clínicas (tenants)
...
CREATE TABLE `usuarios` (
  ...
  `clinica_id` BIGINT NOT NULL DEFAULT 1 COMMENT 'Relación con la clínica',
...
CREATE TABLE `horarios` (
  ...
  CONSTRAINT `fk_horario_usuario`
    ...
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_horario_clinica` FOREIGN KEY (`clinica_id`) REFERENCES `clinicas` (`id_clinica`)
) ENGINE=InnoDB;
...
CREATE TABLE `bloqueos_agenda` (
  ...
  CONSTRAINT `fk_bloqueo_usuario`
    ...
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_bloqueo_clinica` FOREIGN KEY (`clinica_id`) REFERENCES `clinicas` (`id_clinica`)
) ENGINE=InnoDB;
...
CREATE TABLE `clientes` (
  ...
  CONSTRAINT `fk_cliente_clinica` FOREIGN KEY (`clinica_id`) REFERENCES `clinicas` (`id_clinica`)
) ENGINE=InnoDB;
...
CREATE TABLE `documentos_cliente` (
  ...
  `tipo_documento` ENUM('CONSENTIMIENTO_LOPD','CONSENTIMIENTO_TRATAMIENTO','RADIOGRAFIA','RESONANCIA','INFORME_MEDICO','JUSTIFICANTE_PAGO','JUSTIFICANTE_ASISTENCIA','FOTO_PERFIL','OTRO') NOT NULL,
  ...
  CONSTRAINT `fk_documento_cliente`
    ...
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_documento_clinica` FOREIGN KEY (`clinica_id`) REFERENCES `clinicas` (`id_clinica`)
) ENGINE=InnoDB;
...
CREATE TABLE `citas` (
  ...
  `ruta_justificante` VARCHAR(255) NULL,
  ...
  CONSTRAINT `fk_cita_quiropractico`
    ...
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_cita_clinica` FOREIGN KEY (`clinica_id`) REFERENCES `clinicas` (`id_clinica`)
) ENGINE=InnoDB;
...
CREATE TABLE `historial_clinico` (
  ...
  CONSTRAINT `fk_historial_quiropractico`
    ...
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_historial_clinica` FOREIGN KEY (`clinica_id`) REFERENCES `clinicas` (`id_clinica`)
) ENGINE=InnoDB;
...
CREATE TABLE `servicios` (
  ...
  CONSTRAINT `fk_servicio_clinica` FOREIGN KEY (`clinica_id`) REFERENCES `clinicas` (`id_clinica`)
) ENGINE=InnoDB;
...
CREATE TABLE `pagos` (
  ...
  CONSTRAINT `fk_pago_cliente`
    ...
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_pago_clinica` FOREIGN KEY (`clinica_id`) REFERENCES `clinicas` (`id_clinica`)
) ENGINE=InnoDB;
...
CREATE TABLE `bonos_activos` (
  ...
  CONSTRAINT `fk_bono_pago`
    ...
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_bono_clinica` FOREIGN KEY (`clinica_id`) REFERENCES `clinicas` (`id_clinica`)
) ENGINE=InnoDB;
...
CREATE TABLE `grupos_familiares` (
  ...
  CONSTRAINT `fk_grupo_clinica` FOREIGN KEY (`clinica_id`) REFERENCES `clinicas` (`id_clinica`)
) ENGINE=InnoDB;
...
CREATE TABLE `consumos_bono` (
  ...
  CONSTRAINT `fk_consumo_bono`
    ...
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_consumo_clinica` FOREIGN KEY (`clinica_id`) REFERENCES `clinicas` (`id_clinica`)
) ENGINE=InnoDB;
...
CREATE TABLE `auditoria` (
  ...
  CONSTRAINT `fk_auditoria_usuario`
    ...
    REFERENCES `usuarios` (`id_usuario`)
    ON DELETE SET NULL,
  CONSTRAINT `fk_auditoria_clinica` FOREIGN KEY (`clinica_id`) REFERENCES `clinicas` (`id_clinica`)
) ENGINE=InnoDB;
...
-- 0. CLÍNICAS (Tenants)
-- -----------------------------------------------------
INSERT INTO clinicas (id_clinica, nombre, direccion, activa) VALUES 
(1, 'Clínica Principal - Valencia', 'Calle de las Palmeras, 5', 1),
(2, 'Clínica Centro - Madrid', 'Gran Vía, 10', 1);

-- -----------------------------------------------------
-- 1. USUARIOS (Password para todos: "123")
-- -----------------------------------------------------
INSERT INTO usuarios (id_usuario, username, password_hash, nombre_completo, rol, clinica_id, activo) VALUES 
(1, 'admin', '$2a$10$EuwpvVgMatdy.w/j2M.0.O0/w/y/z.x.y.z.x.y.z.x.y.z.x.y.z', 'Administrador Principal', 'admin', 1, 1),
(2, 'dr_ana', '$2a$10$EuwpvVgMatdy.w/j2M.0.O0/w/y/z.x.y.z.x.y.z.x.y.z.x.y.z', 'Dra. Ana Quiropráctica', 'quiropráctico', 1, 1),
(3, 'laura', '$2a$10$EuwpvVgMatdy.w/j2M.0.O0/w/y/z.x.y.z.x.y.z.x.y.z.x.y.z', 'Laura Recepción', 'recepción', 1, 1);

-- Nota: Hash '123' actualizado
...
-- 2. CLIENTES
-- -----------------------------------------------------
INSERT INTO clientes (id_cliente, nombre, apellidos, fecha_nacimiento, telefono, email, direccion, clinica_id, notas_privadas) VALUES 
(1, 'Juan', 'Pérez García', '1985-05-15', '600111222', 'juan.perez@email.com', 'Calle Mayor 1', 1, 'Paciente con dolor lumbar crónico.'),
(2, 'María', 'López Sánchez', '1990-10-20', '600333444', 'maria.lopez@email.com', 'Av. Libertad 25', 1, 'Prefiere citas por la tarde.');


-- -----------------------------------------------------
-- 3. HORARIOS (Dra. Ana - ID 2 / Clínica 1)
-- -----------------------------------------------------
INSERT INTO horarios (id_usuario_quiro, clinica_id, dia_semana, hora_inicio, hora_fin) VALUES 
(2, 1, 1, '09:00:00', '13:00:00'),
(2, 1, 1, '16:00:00', '20:00:00'),
(2, 1, 2, '09:00:00', '14:00:00'),
(2, 1, 3, '09:00:00', '13:00:00'),
(2, 1, 3, '16:00:00', '20:00:00'),
(2, 1, 4, '09:00:00', '13:00:00'),
(2, 1, 4, '16:00:00', '20:00:00'),
(2, 1, 5, '09:00:00', '13:00:00');


-- -----------------------------------------------------
-- 4. SERVICIOS
-- -----------------------------------------------------
INSERT INTO servicios (id_servicio, clinica_id, nombre_servicio, precio, tipo, sesiones_incluidas, activo) VALUES 
(1, 1, 'Primera Visita + Ajuste', 50.00, 'sesion_unica', NULL, 1),
(2, 1, 'Ajuste Regular', 35.00, 'sesion_unica', NULL, 1),
(3, 1, 'Bono 10 Sesiones', 300.00, 'bono', 10, 1),
(4, 1, 'Bono 5 Sesiones', 160.00, 'bono', 5, 1);


-- -----------------------------------------------------
-- 5. BLOQUEOS DE AGENDA (Pruebas)
-- -----------------------------------------------------
INSERT INTO bloqueos_agenda (id_usuario_quiro, clinica_id, fecha_hora_inicio, fecha_hora_fin, motivo) VALUES 
(2, 1, '2025-11-24 12:00:00', '2025-11-24 13:00:00', 'Cita Médica Personal');

INSERT INTO bloqueos_agenda (id_usuario_quiro, clinica_id, fecha_hora_inicio, fecha_hora_fin, motivo) VALUES 
(NULL, 1, '2025-12-25 00:00:00', '2025-12-25 23:59:59', 'Navidad');


-- -----------------------------------------------------
-- 6. PAGOS Y BONOS (María compra un Bono de 10)
-- -----------------------------------------------------
INSERT INTO pagos (id_pago, id_cliente, clinica_id, monto, metodo_pago, fecha_pago, id_servicio_pagado, notas) VALUES 
(1, 2, 1, 300.00, 'tarjeta', NOW(), 3, 'Compra de bono inicial');

INSERT INTO bonos_activos (id_bono_activo, id_cliente, clinica_id, id_servicio_comprado, id_pago_origen, fecha_compra, sesiones_totales, sesiones_restantes, fecha_caducidad) VALUES 
(1, 2, 1, 3, 1, CURDATE(), 10, 10, DATE_ADD(CURDATE(), INTERVAL 1 YEAR));


-- -----------------------------------------------------
-- 7. CITAS (Escenarios de prueba)
-- -----------------------------------------------------
INSERT INTO citas (id_cliente, id_quiropractico, clinica_id, fecha_hora_inicio, fecha_hora_fin, estado, notas_recepcion) VALUES 
(1, 2, 1, '2025-11-10 10:00:00', '2025-11-10 10:30:00', 'completada', 'Vino con dolor agudo.');

INSERT INTO citas (id_cliente, id_quiropractico, clinica_id, fecha_hora_inicio, fecha_hora_fin, estado, notas_recepcion) VALUES 
(1, 2, 1, '2025-11-24 09:00:00', '2025-11-24 09:30:00', 'programada', 'Revisión semanal.');

INSERT INTO citas (id_cliente, id_quiropractico, clinica_id, fecha_hora_inicio, fecha_hora_fin, estado, notas_recepcion) VALUES 
(2, 2, 1, '2025-11-24 09:30:00', '2025-11-24 10:00:00', 'programada', 'Usar bono.');


-- -----------------------------------------------------
-- 8. HISTORIAL, CONSUMOS Y GRUPOS
-- -----------------------------------------------------
INSERT INTO historial_clinico (id_cita, id_cliente, id_quiropractico, clinica_id, fecha_sesion, notas_subjetivo, notas_objetivo, ajustes_realizados, plan_futuro) VALUES 
(1, 1, 2, 1, '2025-11-10 10:00:00', 'Paciente reporta dolor 7/10 en zona lumbar.', 'Contractura visible en L4-L5.', 'Ajuste lumbar lateral.', 'Volver en 1 semana.');

INSERT INTO grupos_familiares (id_cliente_propietario, id_cliente_beneficiario, clinica_id, relacion) 
VALUES (1, 2, 1, 'Padre-Hija');
